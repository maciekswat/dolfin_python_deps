...
#platform-dependent variables - used in making code cross-platform
...
def recompile(modulename, module_path, new_compilation_checksum,
    ...
    try:
        ...
        if build_system == "distutils":
            ...
            cmd = ['cmake', '-G', CMAKE_GENERATOR, '-DDEBUG=TRUE' ,'.']
            # # # cmd = "cmake -DDEBUG=TRUE .";
            
            
            # print 'CURRENT DIR=',os.getcwd()
            # print 'compilation_checksum_filename=',compilation_checksum_filename
            # print 'cmd=',cmd 
            instant_debug("cmd = %s" % cmd)
            ret, output = get_status_output(cmd)
            # print 'ret=',ret
...
def build_module(modulename=None, source_directory=".",
    ...
    if modulename is None:
        ...
        if module: return module
        print 'module_name=',modulename
        print 'while in modulename cwd=',os.getcwd()    
        cache_dir_root,cache_dir_base_name=os.path.split(cache_dir)
        
        error_dir=os.path.join(cache_dir_root,'error')
        # Make a temporary module path for compilation
        module_path = os.path.join(error_dir, modulename+'_build')  
        # module_path = os.path.join(get_temp_dir(), modulename)  # this causes problems with too long filenames
        # instant_assert(not os.path.exists(module_path),
            # "In instant.build_module: Not expecting module_path to exist: '%s'"\
            # % module_path)
        if os.path.exists(module_path):# will remove existing module build directory and start over
            print 'found existing module build path: ',module_path
    ...
    try:
        ...
        if use_cache:
            ...
            module_path_in_build_dir=module_path
        ...
        if module_path_in_build_dir is not None:
            clean_after_build(build_dir=module_path_in_build_dir)
        

    instant_error("In instant.build_module: Should never reach this point!")
    # end build_module

def clean_after_build(build_dir=None):
    try:
        shutil.rmtree(build_dir)
    except Exception,e:
        print 'Problems cleaning after build - directory: ',build_dir
        raise e #rethrowing exception 
